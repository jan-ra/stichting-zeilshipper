    ...
    <div style={{ paddingTop: 68 }}>
      <div style={{ background: '#0f2238', padding: '72px 2rem 60px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ fontSize: 11, color: '#c19a52', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 16 }}>{t('media.badge')}</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '2rem', alignItems: 'flex-end' }} className="media-header">
            <div>
              <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(26px, 4vw, 48px)', color: '#f4ede1', fontWeight: 400, marginBottom: 16 }}>{t('media.title')}</h1>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: 11, color: '#c19a52', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{t('media.filesLabel')}</div>
            </div>
          </div>
      ...
      <div style={{ background: '#0a1a2e', padding: '64px 2rem', borderBottom: '1px solid rgba(193,154,82,0.15)' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', alignItems: 'center' }} className="grid-2">
            ...
            <div>
              <div style={{ fontSize: 11, color: '#c19a52', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 12 }}>{t('media.promotionLabel')}</div>
              <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 30, color: '#f4ede1', fontWeight: 400, marginBottom: 16 }}>{t('media.featuredTitle')}</h2>
      ...
      <div style={{ background: '#f4ede1', padding: '64px 2rem' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ fontSize: 11, color: '#c19a52', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 28 }}>
            {filter === 'all' ? t('media.allFiles') : catLabels[filter]} — {filtered.length} items
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 2 }}>
            {filtered.map(item => (
              ...
              >
                ...
                <div>
                  <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 17, color: '#0f2238', marginBottom: 6, lineHeight: 1.3 }}>{tc(item, 'title')}</div>
                  <div style={{ fontSize: 13, color: 'rgba(15,34,56,0.5)', marginBottom: 10 }}>{tc(item, 'description')}</div>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', alignItems: 'center' }} className="grid-2">
            ...
            <div>
              ...
              <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, color: '#f4ede1', fontWeight: 400, marginBottom: 16 }}>{t('media.podcastTitle')}</h2>
              <p style={{ fontSize: 14, color: 'rgba(244,237,225,0.55)', lineHeight: 1.8, marginBottom: 24 }}>
                {t('media.podcastBody')}
              </p>
              <button style={{ background: '#c19a52', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, letterSpacing: '0.08em', color: '#0f2238', padding: '11px 22px', borderRadius: 2 }}>
      ...
      <div style={{ background: '#0f2238', padding: '64px 2rem', textAlign: 'center' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          <div style={{ fontSize: 11, color: '#c19a52', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 16 }}>{t('media.pressBadge')}</div>
