...
export default function UNESCOPage() {
  ...
  const criteria = UNESCO_PAGE.criteria.map(c => ({
    ...c,
    title:    tc(c, 'title'),
    body:     tc(c, 'body'),
      ...
      <div style={{ background: '#f4ede1', padding: '80px 2rem' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '12px 20px' }}>
            {PARTNERS.map(p => (
              p.url
                ? <a key={p.id} href={p.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(15,34,56,0.2)', padding: p.logo ? '6px 14px' : '8px 18px', borderRadius: 2, minWidth: 100 }}>
                    {p.logo
                      ? <img src={asset(p.logo.src)} alt={p.name} style={{ height: 40, maxWidth: 120, objectFit: 'contain', display: 'block' }} />
                      : <span style={{ fontSize: 13, color: '#0f2238' }}>{p.name}</span>}
                  </a>
                : <span key={p.id} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(15,34,56,0.2)', padding: p.logo ? '6px 14px' : '8px 18px', borderRadius: 2, minWidth: 100 }}>
                    {p.logo
                      ? <img src={asset(p.logo.src)} alt={p.name} style={{ height: 40, maxWidth: 120, objectFit: 'contain', display: 'block' }} />
                      : <span style={{ fontSize: 13, color: '#0f2238' }}>{p.name}</span>}
                  </span>
