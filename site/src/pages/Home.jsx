    ...
    <div>
              ...
              {HOME_PAGE.scrollPhotos.length > 0
                ? [...HOME_PAGE.scrollPhotos, ...HOME_PAGE.scrollPhotos].map((photo, i) => (
                    <div key={i} style={{ flexShrink: 0, width: 260, height: '100%', overflow: 'hidden' }}>
                      <img src={asset(photo.src)} alt={photo.alt || ''} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', filter: 'brightness(0.82) saturate(0.88)' }} />
                    </div>
                  ))
                : null}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', padding: '312px 64px 80px 4rem', position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', bottom: 36, left: '4rem', display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <span style={{ fontSize: 10, color: 'rgba(244,237,225,0.3)', letterSpacing: '0.2em', textTransform: 'uppercase' }}>{tc(HOME_PAGE, 'scrollHint')}</span>
              </div>
            </div>
              ...
              <Fragment key={i}>
                ...
                {ch.photo && (
                  <div style={{ height: 220, overflow: 'hidden', position: 'relative', flexShrink: 0 }}>
                    <img src={asset(ch.photo.src)} alt={ch.photo.alt || ''} style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: ch.photoPosition || 'center center', display: 'block', filter: 'brightness(0.82) saturate(0.88)' }} />
      ...
      <div style={{ background: '#0f2238', padding: '80px 2rem' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }} className="grid-2">
          <div>
            <div style={{ fontSize: 10, color: '#c19a52', letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 18 }}>{tc(HOME_PAGE, 'unescoSectionBadge')}</div>
      ...
      <div style={{ background: '#f4ede1', padding: '100px 2rem' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ fontSize: 10, color: '#c19a52', letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 16 }}>{tc(HOME_PAGE, 'projectsBadge')}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2px' }}>
            {projects.map((c, i) => (
              <div key={i} onClick={() => navigate(c.action)} style={{ background: '#0f2238', padding: '40px 36px', cursor: 'pointer', transition: 'background 0.3s' }}
                ...
                <div style={{ marginTop: 24, fontSize: 12, color: '#c19a52', letterSpacing: '0.06em' }}>{tc(HOME_PAGE, 'projectsReadMore')}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ background: '#0a1a2e', padding: '100px 2rem' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'start' }} className="grid-2">
          <div>
            <div style={{ fontSize: 10, color: '#c19a52', letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 18 }}>{tc(HOME_PAGE, 'oralBadge')}</div>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(24px, 3vw, 40px)', color: '#f4ede1', fontWeight: 400, marginBottom: 20, lineHeight: 1.2 }}>
              {tc(HOME_PAGE, 'oralTitle')}
            </h2>
            <p style={{ fontSize: 15, color: 'rgba(244,237,225,0.6)', lineHeight: 1.85, marginBottom: 20 }}>{tc(HOME_PAGE, 'oralPara1')}</p>
            <p style={{ fontSize: 15, color: 'rgba(244,237,225,0.6)', lineHeight: 1.85, marginBottom: 20 }}>{tc(HOME_PAGE, 'oralPara2')}</p>
            <p style={{ fontSize: 15, color: 'rgba(244,237,225,0.6)', lineHeight: 1.85, marginBottom: 32 }}>{tc(HOME_PAGE, 'oralPara3')}</p>
      ...
      <div style={{ background: '#0f2238', padding: '100px 2rem' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }} className="grid-2">
          ...
          <div>
            <div style={{ fontSize: 10, color: '#c19a52', letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 16 }}>{tc(HOME_PAGE, 'mediaSpotlightBadge')}</div>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 32, color: '#f4ede1', fontWeight: 400, marginBottom: 18, lineHeight: 1.2 }}>{tc(HOME_PAGE, 'mediaSpotlightTitle')}</h2>
            <p style={{ fontSize: 15, color: 'rgba(244,237,225,0.55)', lineHeight: 1.85, marginBottom: 28 }}>{tc(HOME_PAGE, 'mediaSpotlightBody')}</p>
      ...
      <div style={{ background: '#f4ede1', padding: '100px 2rem' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 52 }}>
            <div>
              <div style={{ fontSize: 10, color: '#c19a52', letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 12 }}>{tc(HOME_PAGE, 'newsBadge')}</div>
              <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 38, color: '#0f2238', fontWeight: 400 }}>{tc(HOME_PAGE, 'newsTitle')}</h2>
            </div>
            <button onClick={() => navigate('blog')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: '#c19a52', letterSpacing: '0.06em' }}>{tc(HOME_PAGE, 'newsAllCta')}</button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '3rem' }}>
